...
def gerar_resumo_curriculos_batch(lista_markdowns):
    parser = PydanticOutputParser(pydantic_object=ResumoCurriculo)
    format_instructions = parser.get_format_instructions()
    ...
    # inputs: lista de dicts, cada um com {"curriculo": ...}
    inputs = [{"curriculo": md} for md in lista_markdowns]
...
def gerar_score_curriculos_batch(lista_resumos, lista_desc_vaga):
    system_prompt = '''
        Você é um avaliador de currículos com muitos anos de experiência e receberá certas instruções para atribuir uma nota ao curriculo.
        A sua resposta deve ser apenas o número da nota final do candidato com 1 casa decimal de precisão.
        Não responda com nada além da nota final do candidato.
    ...
    # inputs: lista de dicts, cada um com {"resumo": ..., "desc_vaga": ...}
    inputs = [{"resumo": r, "desc_vaga": d} for r, d in zip(lista_resumos, lista_desc_vaga)]
    resultados = chain.batch(inputs)
    # Retorna apenas o conteúdo (score) de cada resultado
    return [r.content for r in resultados]

def gerar_opiniao_curriculos_batch(lista_resumos, lista_desc_vaga):
    system_prompt = '''
        Você é um avaliador de currículos com muitos anos de experiência e receberá certas instruções para atribuir uma nota ao curriculo.
        A sua resposta deve ser apenas o número da nota final do candidato.
        Não responda com nada além da nota final do candidato.
    ...
    inputs = [{"resumo": r, "desc_vaga": d} for r, d in zip(lista_resumos, lista_desc_vaga)]
    resultados = chain.batch(inputs)
    # Retorna apenas o conteúdo (opinião) de cada resultado
    return [r.content for r in resultados]
